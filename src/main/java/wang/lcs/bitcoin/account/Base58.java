...
public class Base58 {
	 ...
	 * checksum in the last 4 bytes of the decoded data to verify that the rest are
	 * correct. The checksum is removed from the returned data.
	 *
	 * @param input
	...
	public static byte[] decodeChecked(String input) throws IllegalArgumentException, NoSuchAlgorithmException {
		...
		byte[] actualChecksum = Arrays.copyOfRange(digest.digest(digest.digest()), 0, 4);
